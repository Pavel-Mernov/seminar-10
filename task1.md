...
24. Выполните поиск по началу названия:
    ...
    *Объясните результат:*
    Оператор **ILIKE** в сочетании с индексом на поле **title** в обратном порядке + **extension** и индекс на **title** внутри *extension* даёт эффективный по времени результат (фактическое время - $0.3$ мсек, ожидаемое - более $0.5$ мсек).
...
25. Создайте свой пример индекса с обратной сортировкой:
    ...
    *Тестовый запрос:*
    ```sql
      EXPLAIN ANALYZE SELECT * FROM t_books WHERE title = 'Book_10001';
    ```
    
    *План выполнения:*
    ![image](https://github.com/user-attachments/assets/b6984b38-e40d-41ee-b535-8d2d9d7b5695)

    
    *Объясните результат:*
    Фактическое время - $0.077$ мсек, ожидаемое время - $0.267$ мсек. **Фактическое время меньше ожидаемого.**

    Созданный мной индекс в сочетании с созданными выше индексами на **title** создаёт эффективный инструмент оптимизации поиска, не создавая конфликтов с другими индексами на то же поле **title**.
